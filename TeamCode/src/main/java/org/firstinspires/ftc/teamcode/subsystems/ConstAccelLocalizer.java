...
public class ConstAccelLocalizer extends SubSystem{
    Localizer localizer;
...
//        });
    ...
    @Override
    public void loop() {
        localizer.update(new Point(parallelWheelDistance, timer.seconds()), new Point(perpendicularWheelDistance, timer.seconds()), new Point(imuAngle, timer.seconds()));
...
//        imuAction.queueAction();
    ...
    public TelemetryPacket dashboard(TelemetryPacket packet) {
        ...
        DashboardUtil.drawFullPoseHistory(packet.fieldOverlay(), localizer.getPrevPositions());
        ...
        Vector2d robotVelocity = localizer.getVelocity().getVector2d();
